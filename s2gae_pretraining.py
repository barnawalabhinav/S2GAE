...
from torch_geometric.data import Data
from zinc import MoleculeDataset
import warnings

# Suppress specific warnings
warnings.filterwarnings("ignore", category=UserWarning, message="It is not recommended to directly access the internal storage format `data` of an 'InMemoryDataset'")
...
def train(model, predictor, data, edge_index, optimizer, args):
    ...
    for perm in DataLoader(range(pos_train_edge.size(0)), args.batch_size,
        ...
        # print(o)
        # o = o+1
        # print(pos_train_edge.size(0))
        optimizer.zero_grad()
        h = model(data.x.float(), adj)
...
def test(model, predictor, data, pos_test_edge, neg_test_edge, batch_size):
    ...
    h = model(data.x.float(), data.full_adj_t)
...
def main():
    ...
    parser.add_argument('--graph_batch',type=int, default = 128)
    ...
    save_path_model = 'weight/s2gaesvm-' + args.use_sage + str(args.graph_batch) +  '_{}_{}'.format(args.dataset, args.mask_type) + '_{}'.format(
        args.num_layers) + '_hidd{}-{}-{}-{}'.format(args.hidden_channels, args.mask_ratio, args.decode_layers,
                                                     args.decode_channels) + '_model.pth'
    save_path_predictor = 'weight/s2gaesvm' + args.use_sage + str(args.graph_batch) +  '_{}_{}'.format(args.dataset,
    ...
    print('Start training with mask ratio={}'.format(args.mask_ratio))
    ...
    for epoch in range(1, 1 + args.epochs):
        ...
        for data_ in data_loader:
            ...
            data_.full_adj_t = SparseTensor.from_edge_index(edge_index).t()
            edge_index, test_edge, test_edge_neg = do_edge_split_nc(edge_index, data_.x.shape[0])
            data_.to(device)
            loss += train(model, predictor, data_, edge_index, optimizer,
                         args)
            
            auc_test += test(model, predictor, data_, test_edge, test_edge_neg,
                           args.batch_size)
            num_batches+=1
        loss = loss/num_batches
        auc_test = auc_test/num_batches
        ...
        if auc_test > best_valid:
            ...
            cnt_wait += 1
        if (epoch%20==0):
            intermediate_path = 'weight/s2gaesvm-' + args.use_sage + str(args.graph_batch) + '_{}_{}'.format(args.dataset, args.mask_type) + '_{}'.format(
        args.num_layers) + '_hidd{}-{}-{}-{}'.format(args.hidden_channels, args.mask_ratio, args.decode_layers,
                                                     args.decode_channels) + '_model'+ str(epoch)+".pth"
            intermediate_path_predictor = 'weight/s2gaesvm' + args.use_sage + str(args.graph_batch)+ '_{}_{}'.format(args.dataset,
                                                                          args.mask_type) + '_{}'.format(
        args.num_layers) + '_hidd{}-{}-{}-{}'.format(args.hidden_channels, args.mask_ratio, args.decode_layers,
                                                     args.decode_channels) + '_pred' + str(epoch) + "pth"
            torch.save(model.state_dict(), intermediate_path)
            torch.save(predictor.state_dict(), intermediate_path_predictor)
